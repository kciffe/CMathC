...
图 1 展示镜像提示在方向和三角形构型前端中的空间响应。图中热图是模型内部特征，不是实测脑电或源定位结果。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{../documents/figures/视觉刺激与前端空间特征.png}
...
这里的 L/R 分别表示左、右模板偏好。实现将空间图按 \(8\times8\) 池化，前端特征每 4 ms 计算一次，再插值到 1 ms 群体动力学网格。图 2 把实际数据、群体状态、五个源代理和传感器输出放在同一计算链中；曲线示意取自一个留出记录折。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{../documents/figures/revision_v3_真实数据与皮层观测模块IO.png}
...
\midrule
\endhead
去除空间位置后左右差异保留比例 & \(1.03\times10^{-5}\) 至 \(2.68\times10^{-5}\) & 当前模型内的空间输入依赖 \\
...
图 3 显示真实与模型右减左差异在三个观测模式上的比较。模型的侧化分量存在，但共同和形状分量与实测波形并未同步；源可加和只核验了代码映射关系，不意味着源代理已经由三个电极唯一识别。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{../documents/figures/实测与模型左右差异模式.png}
\caption{实测与模型左右差异模式}
\end{figure}
...
八个“记录 \(\times\) 左右提示”条件的平均 NRMSE 为 \(1.810\)，即误差规模大于实测 ERP 自身 RMS。右减左差异波相关均值为 \(-0.147\)，说明模型预测的差异波与实测差异波在形状上未呈现正相关；当前参数下模型未复现观测到的左右差异波形。A-2 和 B-2 的条件平均 NRMSE 低于 1，但 A-1 为 4.313，B-1 为 1.383；总体结果不支持当前模型已稳定复现真实 ERP。图 4 给出各记录和电极的完整留出曲线。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{../documents/figures/留一记录实测与模型ERP.png}
\caption{留一记录实测 ERP 与模型预测}
\end{figure}
...
图 5 逐折显示该特征的平衡准确率与 \(50\%\) 参考线。记录间差异和低宏平均值说明现有特征及数据条件下的跨记录泛化有限。

\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{../documents/figures/九维ERP特征_留一记录分类.png}
\caption{九维 ERP 特征留一记录分类}
\end{figure}
