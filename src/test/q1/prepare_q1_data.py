...
def read_rad(path, station):
    ...
    for line in open(path, encoding='utf8', errors='ignore'):
        ...
        if beam:
            ...
            if len(p) >= 4:
                rows.append([
                    station,time,mode_id,beam,
                    num(p[0]),num(p[1]),
                    num(p[2]),num(p[3])
                ])

    return pd.DataFrame(rows, columns=[
        'station','time','mode_id','beam','height',
        'spectral_width','snr',
        'radial_velocity'
    ])
...
def read_mwr(path, station):
    ...
    names={
        11:'temperature',        # 11号类型：温度
        12:'water_vapor_density',# 12：水汽密度
        13:'relative_humidity',  # 13：相对湿度
        14:'liquid_water'        # 14：液态水含量
    }
        ...
        for (s,t),idx in data.groupby(
            ...
            if len(mw)>1:
                ...
                data.loc[idx,col]=np.interp(
                    data.loc[idx,'height'],  # ① 目标高度：风廓线所有高度
                    mw.height,               # ② 原始x：MWR自身高度
                    mw[col]                  # ③ 原始y：MWR对应要素值
                )
