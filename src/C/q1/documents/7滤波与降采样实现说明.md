...
下列图均来自 `output/7filter_downsample/`，按数据集静态排版。每组依次展示滤波前、滤波后、降采样前（已滤波）和降采样后的结果，便于核对每个处理阶段；PDF 中每个数据集从新页开始，避免标题与首张图分离。图片仍保留在任务输出目录，文档只引用、不复制。
...
![VisualCogA_Task-1 滤波前 Trial](../output/7filter_downsample/before_filter/VisualCogA_Task-1_before_filter.png)

**滤波后**

![VisualCogA_Task-1 滤波后 Trial](../output/7filter_downsample/after_filter/VisualCogA_Task-1_after_filter.png)

**降采样前（已滤波）**

![VisualCogA_Task-1 降采样前 Trial](../output/7filter_downsample/before_downsample/VisualCogA_Task-1_before_downsample.png)

**降采样后**

![VisualCogA_Task-1 降采样后 Trial](../output/7filter_downsample/after_downsample/VisualCogA_Task-1_after_downsample.png)
...
![VisualCogA_Task-2 滤波前 Trial](../output/7filter_downsample/before_filter/VisualCogA_Task-2_before_filter.png)

**滤波后**

![VisualCogA_Task-2 滤波后 Trial](../output/7filter_downsample/after_filter/VisualCogA_Task-2_after_filter.png)

**降采样前（已滤波）**

![VisualCogA_Task-2 降采样前 Trial](../output/7filter_downsample/before_downsample/VisualCogA_Task-2_before_downsample.png)

**降采样后**

![VisualCogA_Task-2 降采样后 Trial](../output/7filter_downsample/after_downsample/VisualCogA_Task-2_after_downsample.png)
...
![VisualCogB_Task-1 滤波前 Trial](../output/7filter_downsample/before_filter/VisualCogB_Task-1_before_filter.png)

**滤波后**

![VisualCogB_Task-1 滤波后 Trial](../output/7filter_downsample/after_filter/VisualCogB_Task-1_after_filter.png)

**降采样前（已滤波）**

![VisualCogB_Task-1 降采样前 Trial](../output/7filter_downsample/before_downsample/VisualCogB_Task-1_before_downsample.png)

**降采样后**

![VisualCogB_Task-1 降采样后 Trial](../output/7filter_downsample/after_downsample/VisualCogB_Task-1_after_downsample.png)
...
![VisualCogB_Task-2 滤波前 Trial](../output/7filter_downsample/before_filter/VisualCogB_Task-2_before_filter.png)

**滤波后**

![VisualCogB_Task-2 滤波后 Trial](../output/7filter_downsample/after_filter/VisualCogB_Task-2_after_filter.png)

**降采样前（已滤波）**

![VisualCogB_Task-2 降采样前 Trial](../output/7filter_downsample/before_downsample/VisualCogB_Task-2_before_downsample.png)

**降采样后**

![VisualCogB_Task-2 降采样后 Trial](../output/7filter_downsample/after_downsample/VisualCogB_Task-2_after_downsample.png)
