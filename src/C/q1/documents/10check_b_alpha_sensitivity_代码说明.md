...
2. `{dataset}_alpha_ERP_sensitivity.png`：每个 B 组一张 2×3 图。行是左右条件，列是 F3/Fz/F4；不同颜色表示 α，图例附该条件保留 Trial 数。标题下方的全图图例同时说明 α 曲线和两个候选窗：蓝色阴影表示提示后 P300 候选窗 `0.25～0.50 s`，粉色阴影表示目标后 P300 候选窗 `2.45～2.70 s`（相对提示 onset）。蓝色虚线和点线分别标出提示 onset `t=0` 与目标 onset `t=2.2 s`；所有面板均显示横轴刻度数值。图中若多个 α 的保留集合相同，其曲线会完全重合，视觉上表现为少于四条不同曲线。
