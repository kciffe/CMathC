...
- 横轴：相对提示 onset 的时间，显示 `−0.2～3.0 s`；蓝色竖直虚线和点线分别标出提示 onset `t=0` 与目标 onset `t=2.20 s`；
- 蓝色阴影：提示后 P300 候选窗 `0.25～0.50 s`；粉色阴影：目标后 P300 候选窗 `2.45～2.70 s`；
- 曲线：使用第 7 阶段已滤波、已降采样的波形原值，不做本脚本内的基线校正或 ERP 平均；
- 图例：给出 Trial 编号和 SQI；标题及各行标注阈值与实际绘出的 Trial 数。
- 标题下方的全图图例用蓝、粉色块说明两个阶段的 P300 候选窗，并标出提示开始（0 s，蓝色虚线）和目标显示开始（2.20 s，蓝色点线）；每个面板都显示横轴刻度数值。
