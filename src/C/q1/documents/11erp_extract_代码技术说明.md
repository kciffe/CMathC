...
每个数据集输出一张 `{dataset}_ERP.png`，包含左右两个面板；每个面板叠加 F3、Fz、F4 三条 ERP 曲线，并在标题中显示该条件的 Trial 数。蓝色竖直虚线和点线分别标出提示 onset `t=0` 和目标 onset `t=2.20 s`，图题下方的全图图例也用线型标注这两个阶段开始点。蓝色阴影标出提示后 P300 候选窗 `0.25～0.50 s`，粉色阴影标出目标后 P300 候选窗 `2.45～2.70 s`（相对提示 onset）；左右面板均显示横轴刻度数值。
