...
def register_frontend(tau_a, resolution, frontend):
    """Seed the immutable cache with a previously audited deterministic frontend."""
    key = (float(tau_a), int(resolution))
    _SEEDED_FRONTENDS[key] = frontend
    _cached_left_frontend.cache_clear()
...
def _cached_left_frontend(tau_a, resolution):
    ...
    key = (float(tau_a), int(resolution))
    if key in _SEEDED_FRONTENDS:
        return _SEEDED_FRONTENDS[key]
...
def _make_candidate_evaluator(tau_a, fronts, cases, weights,
    ...
    def evaluate_candidate(x):
        ...
        for case in cases:
            ...
            pred = _interpolate_prediction(processed, processed_time, case["time_ms"])
        ...
        # One shared signed gain is used because the sign of the relative-unit
        # E-I source proxy is not an anatomical dipole orientation.
        amplitude = float(numer / denom)
...
def _optimize_start(evaluate_candidate, raw_start, lower, upper, max_nfev):
    ...
    local = {"evaluations": 0, "best": None, "budget_exhausted": False}
    ...
    if local["budget_exhausted"]:
        opt.success = False
        opt.status = 1
        opt.message = "objective evaluation budget exhausted; candidate is best finite point evaluated"
...
def fit_model(train_cases, model_spec=None, seed=config.SEED, resolution=64,
              ...
              feature_route="opponent", observation_rank=3, drive_scales=None):
    """Fit tau_s/g_i per fixed tau_a profile and one shared signed scale.
    ...
    if feature_route not in ("opponent", "legacy") or observation_rank != 3:
        raise ValueError("invalid feature route or observation rank")
    ...
    if observation_rank != 3:
        raise ValueError("revision_v3 uses all three electrode observations")
    if drive_scales is None:
        reference = _cached_left_frontend(float(config.TAU_ADAPT_DEFAULT), int(resolution))
        mirrored = mirror_stage1_frontend(reference, right_stimulus)
        drive_scales = calibrate_drive_scales({"left": reference, "right": mirrored}, feature_route)
    ...
    return FitResult(records, values, float(amplitude), float(loss), bool(success), state,
                     ...
                     feature_route, observation_rank)


def fit_leave_one_record(cases, resolution=64, feature_route="opponent", observation_rank=3,
                         progress=False, max_nfev=config.MAX_NFEV_PER_START,
                         drive_scales=None):
    ...
    if drive_scales is None:
        reference = _cached_left_frontend(float(config.TAU_ADAPT_DEFAULT), int(resolution))
        right = mirror_stage1_frontend(reference, load_stimulus("Stage1", "right"))
        drive_scales = calibrate_drive_scales({"left": reference, "right": right}, feature_route)
    results = {}
    for heldout in records:
        if progress:
            print(f"  fitting held-out {heldout} ({feature_route}, three sensors)",
                  flush=True)
        train = [c for c in eligible if c["dataset"] != heldout]
        results[heldout] = fit_model(train, resolution=resolution, progress=progress,
                                     feature_route=feature_route,
