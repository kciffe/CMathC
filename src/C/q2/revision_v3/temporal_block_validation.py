...
def _write_report(block_rows, stability_rows, lda_rows, stimulus_rows, out):
    ...
    lines += ["", "逐像素相减适合提取三角出现/消失的亮度增量：若圆圈在两个画面中相同，它会在差分中抵消。但 v3 把该差分当作空间输入，并从零状态启动；因此它没有表示提示前圆圈已经建立的 LGN/皮层活动，也没有表示提示后圆圈继续显示。差分可作为瞬态输入，不能代替完整场景驱动的神经状态。", "",
              ...
              "2. 旧版六源命名曾混用视觉半区、三角模板偏好与解剖半球。本次已改为：早期/构型视觉半区按对侧路由到四个半球代理源；左右模板偏好只形成一个带符号的双侧中线对抗源。见 `audit_source_mapping_geometry.py` 的逐通道路由矩阵。",
              "3. 旧版几何曾把源也投影到 90 mm 外表面。本次已将传感器/参考保留在 90 mm 外表面、五个源置于半径 65.5–74.1 mm 的内部坐标。前向公式仍是均匀无限导体点偶极近似，不能称为有限球体头模型或真实头模型导联场。",
