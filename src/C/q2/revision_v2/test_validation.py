...
def test_feature_extraction_returns_three_windows_times_two_modes():
    ...
    features = extract_features(trials, time_ms, mode_count=2)

    assert features.shape == (2, 6)
    assert np.all(features[0, 3:] < 0)
    assert np.all(features[1, 3:] > 0)
...
def test_erp_metrics_separate_observable_fit_from_unexplained_u2():
    ...
    modes = np.vstack([np.linspace(0.0, 1.0, len(time_ms)),
                       ...
                       np.linspace(0.5, 0.0, len(time_ms))])
    ...
    assert metrics["mode_correlations"][:2] == pytest.approx([1.0, 1.0])
    assert metrics["real_modes_rms"][2] > metrics["predicted_modes_rms"][2]
...
def test_repeated_condition_runner_reuses_deterministic_frontend(monkeypatch):
    ...
    monkeypatch.setattr(evaluate, "simulate_forward", lambda front, params, amplitude, **kwargs:
