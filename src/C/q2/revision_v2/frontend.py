...
def _center_response(frame, center_rc, radius=1):
    """Read a template response in a fixed patch around its configured center."""
    center = np.rint(center_rc).astype(int)
    r0, c0 = center
    r1, c1 = r0 - radius, c0 - radius
    r2, c2 = r0 + radius + 1, c0 + radius + 1
    patch = frame[max(0, r1):min(frame.shape[0], r2),
                  max(0, c1):min(frame.shape[1], c2)]
...
def simulate_frontend(stimulus, events=None, params=None, time_ms=None,
    ...
    sample_times = np.arange(time_ms[0], time_ms[-1] + feature_stride_ms * 0.5,
                             feature_stride_ms)
    sample_indices = set(int(np.argmin(np.abs(time_ms - value))) for value in sample_times)
    sample_indices.update((0, t_count - 1))
    centers = {
        "left": map_original_coordinates(templates["left"].center_rc, resolution),
        "right": map_original_coordinates(templates["right"].center_rc, resolution),
    }
    ...
    sample_indices.update(audit_indices)
    if stimulus.stage == "Stage1":
        sample_indices.update(int(np.argmin(np.abs(time_ms - t))) for t in (0.0, 200.0))
    sample_indices = np.asarray(sorted(sample_indices), dtype=int)
    sampled_times = time_ms[sample_indices]
    sample_count = len(sample_indices)
    gabor_sampled = np.zeros((4, 8, 8, sample_count), dtype=np.float32)
    b_sampled = np.zeros((8, 8, sample_count), dtype=np.float32)
    hl_sampled = np.zeros_like(b_sampled)
    hr_sampled = np.zeros_like(b_sampled)
    response_left_sampled = np.zeros(sample_count, dtype=np.float32)
    response_right_sampled = np.zeros(sample_count, dtype=np.float32)
    audit_maps = {}
    ...
    for start in range(0, sample_count, batch_size):
        ...
        block = relay[:, :, sample_indices[start:stop]]
        ...
        for local_t in range(stop - start):
            ...
            sample_i = start + local_t
            gi = int(sample_indices[sample_i])
            response_left_sampled[sample_i] = _center_response(hl, centers["left"])
            response_right_sampled[sample_i] = _center_response(hr, centers["right"])
            if gi in audit_indices:
                audit_maps[audit_indices[gi]] = {"V1": v.copy(), "B": bmap.copy(),
                                                 "H_L": hl.copy(), "H_R": hr.copy(),
                                                 "shape": (hl + hr).copy(),
                                                 "direction": (hr - hl).copy()}
            for oi in range(4):
                gabor_sampled[oi, :, :, sample_i] = _pool8(v[oi])
            b_sampled[:, :, sample_i] = _pool8(bmap)
            hl_sampled[:, :, sample_i] = _pool8(hl)
            hr_sampled[:, :, sample_i] = _pool8(hr)
        del v_block
    def interpolate_samples(values):
        flat = np.asarray(values).reshape(-1, sample_count)
        expanded = np.stack([np.interp(time_ms, sampled_times, row) for row in flat])
        return expanded.reshape(*values.shape[:-1], t_count).astype(np.float32)
    gabor_small = interpolate_samples(gabor_sampled)
    b_small = interpolate_samples(b_sampled)
    h_left = interpolate_samples(hl_sampled)
    h_right = interpolate_samples(hr_sampled)
    response_left = interpolate_samples(response_left_sampled)
    response_right = interpolate_samples(response_right_sampled)
    ...
    shape = h_left + h_right
    direction = h_right - h_left
    dir_left = np.maximum(response_left - response_right, 0.0)
    dir_right = np.maximum(response_right - response_left, 0.0)
    return {
        ...
        "R_L": response_left, "R_R": response_right,
...
def mirror_stage1_frontend(left_frontend, right_stimulus):
    ...
    result["shape"] = left_frontend["shape"][:, ::-1, :].copy()
    result["direction"] = -left_frontend["direction"][:, ::-1, :].copy()
    result["R_L"] = left_frontend["R_R"].copy()
    result["R_R"] = left_frontend["R_L"].copy()
    result["dir_left"] = left_frontend["dir_right"].copy()
    result["dir_right"] = left_frontend["dir_left"].copy()
    ...
    result["audit_maps"] = {
        time: {"V1": maps["V1"][orientation_mirror, :, ::-1].copy(),
               ...
               "H_L": maps["H_R"][:, ::-1].copy(),
               "H_R": maps["H_L"][:, ::-1].copy(),
               "shape": maps["shape"][:, ::-1].copy(),
               "direction": -maps["direction"][:, ::-1].copy()}
...
def temporal_stride_audit(stimulus=None, tau_a=80.0, resolution=64,
                          stride_ms=config.FRONTEND_FEATURE_STRIDE_MS):
    """Compare interpolated frontend features with a 1-ms feature reference.

    Returns rows plus the two computed frontends so a downstream caller may
    reuse the coarse result instead of recomputing it.
    """
    stimulus = load_stimulus("Stage1", "left") if stimulus is None else stimulus
    common = {"params": {"tau_a": float(tau_a)}, "resolution": int(resolution)}
    coarse = simulate_frontend(stimulus, feature_stride_ms=float(stride_ms), **common)
    fine = simulate_frontend(stimulus, feature_stride_ms=1.0, **common)
    rows = []
    for name in ("gabor", "B", "H_L", "H_R", "shape", "direction",
                 "R_L", "R_R", "dir_left", "dir_right"):
        a, b = np.asarray(coarse[name], dtype=float), np.asarray(fine[name], dtype=float)
        difference = a - b
        ...
        rows.append({"feature": name, "stride_ms": float(stride_ms),
                     "relative_rmse_to_1ms": float(np.sqrt(np.mean(difference * difference)) / denom)
                         if denom > 1e-12 else float(np.sqrt(np.mean(difference * difference))),
                     "maximum_absolute_error": float(np.max(np.abs(difference))),
                     "correlation_to_1ms": correlation})
    return rows, coarse, fine
