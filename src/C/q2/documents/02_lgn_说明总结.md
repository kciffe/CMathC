...
`*_lgn_on.npy` 与 `*_lgn_off.npy` 保存 TCR 的两类通路响应，`*_lgn_total.npy` 保存两者之和，作为下一层的候选输入。IN 和 TRN 在程序中作为调节 TCR 的内部状态参与计算；当前文件输出与响应图聚焦 TCR，没有单独导出 IN/TRN 时序数组。`LGN空间峰值热图.png` 对 64 通道按方向取平均后显示空间峰值，仅用于展示位置分布；方向细节仍保留在 `*_lgn_total.npy` 中。
