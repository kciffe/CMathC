...
## 八、局部 EEG 特征的探索性最终拟合结果

前五步完成了视觉输入标准化、LGN/皮层正向计算、源到电极观测映射以及真实 EEG 对照。本轮再把真实 Stage1 EEG 的读取目标收窄为一个可直接解释的问题：**左右提示是否能由某个电极的一段局部振幅特征区分？**结果是可以在四份记录中分别找到这样的候选特征。若将“得到一个约 60% 以上、结构简单且可说明的左右读出”作为当前分析目标，这可以作为本轮的**最终探索性拟合结果**。
...
这组结果给出的可解释结论是：在当前清洗数据中，左右提示相关信息不一定表现为三个电极共同出现、贯穿整个 ERP 或集中在单一 P300 窗口的差异；对部分记录而言，区分信息可由一个较短时间窗内的单电极振幅读出。宽时间窗平均可能会稀释这类局部变化。与此同时，最佳电极和时间位置在记录间并不一致：有的出现在提示消失附近，有的出现在更晚时段。因此，结果支持“存在记录内、时间局部化的候选左右特征”，**尚不支持存在跨记录统一的电极—时间机制**。

论文可将其作为前五步正向建模之后的一个**样本内观测读出结果**：正向模型说明空间位置/方向信息如何沿 LGN—皮层—头皮观测链传播；局部阈值模型说明在真实清洗 EEG 中，哪些观测电极和时间片能对左右标签提供有限区分。二者形成“机制模型 + 可解释数据读出”的当前完整结果。准确率是在同一批试次上搜索电极、窗口、方向、阈值并计算得到，存在选择偏差，不能写成留出测试准确率、跨记录泛化性能，也不能单独作为正向机制已被真实 EEG 证实的证据。
