...
def _build_mapping_summary(frame: pd.DataFrame) -> pd.DataFrame:
    data = frame.copy()
    data["cue_response_same_side"] = pd.to_numeric(
        data["cue_response_direction_consistent"], errors="coerce"
    )
...
"""
    code_window_count = int(
        pd.to_numeric(frame["response_declared_code_sample_count_in_analysis_window"], errors="coerce").gt(0).sum()
    )
...
def main() -> None:
    ...
    overall = {
        ...
        "n_cue_intervals_with_any_response_edge": int(pd.to_numeric(trials["channel9_response_event_count_in_cue_interval"], errors="coerce").gt(0).sum()),
        "n_cue_intervals_without_response_edge": int(pd.to_numeric(trials["channel9_response_event_count_in_cue_interval"], errors="coerce").eq(0).sum()),
        "n_trials_with_declared_code_in_analysis_window": int(pd.to_numeric(trials["response_declared_code_sample_count_in_analysis_window"], errors="coerce").gt(0).sum()),
        ...
        "n_direction_consistency_labels": int(trials["cue_response_direction_consistent"].notna().sum()),
