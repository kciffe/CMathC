...
1. **行为事件路径**：从通道8视觉提示和通道9动作标记恢复每试次的线索侧、响应侧、按同号规则判定的正确/错误标签、按 cue+3.0 秒规则判定的及时性、试次区间动作标记计数，以及动作段持续时间。
...
动态模型路径是问题三的主要机制模型；静态特征、行为预测和消融是独立的辅助分析。通道9在行为支路提供应答方向和首次应答边沿；通道8与通道9同号记正确、异号记错误，首次通道9边沿不晚于 cue+3.0 秒记及时。Task-2 的 TgtAct 动作段先出现同号 ±1、后出现同号 ±2 时，仍视为一次应答，起始时刻取首次零到非零边沿。行为标签不作为 EEG 特征或动态状态方程输入，避免标签泄漏。
...
### 3.3 正确性、及时性、区间动作标记和动作段时长

设 \(s_c\in\{-1,+1\}\) 为通道8线索方向，\(s_r\) 为通道9按 DataLabel 声明码解码的响应方向。仅当响应方向成功解码时，按已提供的规则定义正确性指标：

$$
\mathrm{correct}_i=\mathbf 1(s_{r,i}=s_{c,i}),\qquad
\mathrm{incorrect}_i=\mathbf 1(s_{r,i}\ne s_{c,i}).
$$

该规则比较通道8线索方向与通道9按 MAT DataLabel 解码出的响应方向：同为负号或同为正号判为正确，异号判为错误，无法解码 cue 或 response 方向时正确性标签留空。Task-2 若动作段为 -1→-2 或 +1→+2，因两个码同号且处于同一连续非零段，仍依据该动作段判左/右和正确性；不能把幅值改变当成第二次按键。

及时性按用户给定的 cue 后 3.0 秒截止判定。令 \(t_c\) 为通道8 cue 时刻，\(t_{\mathrm{act}}\) 为通道9首次零到非零边沿：

$$
t_{\mathrm{deadline}}=t_c+3.0\text{ s},\qquad
\mathrm{timely}_i=\mathbf 1(t_{\mathrm{act},i}\le t_{\mathrm{deadline},i}).
$$

若 cue 区间记录完整覆盖截止时刻但没有通道9边沿，则标为“截止前无应答/未及时”；若记录在截止前结束，则及时性保留未知。cue 起点至下一 cue 起点的动作边沿数仍单独记录，不与及时标签混为一谈。
...
\(K_i\) 只表示自定义观测窗内的通道9声明码计数，不参与及时性判定。及时性使用 cue+3.0 秒规则；目标至应答的传统反应时仍需真实逐试次目标时刻 \(t_T\)，定义为 \(t_{\mathrm{act}}-t_T\)。当前没有独立目标 onset 标记，因此传统反应时和以它为输入的 DDM 不可靠。cue+2.2 秒只是排程代理，不能替代真实目标事件。
...
| common.py | 读取 MAT、解析 DataLabel、检测提示/动作边沿、Task-2 动作段解码、按同号规则判正确/错误、按 cue+3.0 秒判及时性、建立 Q1 质量映射 | 原始 MAT、可选 Q1 清洗结果、统一配置 | 事件表/试次字段、行为标签、动作持续时间和 QC/映射信息 | 统一事件定义；保证动作码幅值变化不会误增事件数，并区分行为标签、计数窗口和目标相对反应时 |
...
| 08_behavior_event_audit.py | 独立审计通道推导的正确/错误与 cue+3.0 秒及时性；可选合并外部真值、目标时刻、正式截止和记录映射表作交叉核对 | trial_table.csv、原始 MAT、可选 input 下的外部核验表 | behavior_event_audit/behavior_trial_audit.csv、record_metadata_audit.csv、behavior_record_summary.csv、summary JSON | 核验同号正确性和 cue+3.0 秒及时性，保留外部元数据；不把 cue-relative latency 冒充目标相对 RT |
...
| 05_behavior_model.py | 独立预测通道9选择方向和按同号规则生成的正确/错误标签；分记录留出；真实 target-relative RT 不可得时跳过 DDM | trial_table.csv、state_scores.csv | 行为预测 CSV、折间模型比较、behavior_model_status.json、按记录的行为摘要 | 检验 cue、候选任务编号及 EEG 状态对选择方向/正确性的跨记录预测价值；这是辅助行为分析，不是诊断 |
...
说明：08 是同号正确性、cue+3.0 秒及时性、动作标记和外部元数据的审计模块。通道8/9按既定规则直接生成正确/错误；及时性按首次动作边沿和 cue+3.0 秒截止生成。cue−1 至 cue+5 秒只用于窗口内标记计数。真实目标 onset 仍缺失，因此传统 target-to-response RT 和 DDM 仍受限。03/04/05/06/07 构成辅助特征分析支路；dynamic_validation 才是将 Q2 前向过程与动态认知状态结合的主模型检验。
...
05 对通道9选择方向做 L2 正则逻辑回归，分别比较 cue+候选任务编号、V/H/P 状态、以及两类变量合并；另以通道8/9同号正确、异号错误指标为标签，比较 cue 侧、EEG 状态、cue 侧+EEG 状态的记录留出预测。行为标签只作预测目标，不进入 EEG 特征或动态认知状态方程。06 用相同的记录留出框架从静态 EEG 特征预测通道8提示侧。
...
这里的平衡准确率评价的是模型对按已给同号规则定义的正确/错误标签的留出预测能力。该标签是本项目采用的操作性判据；如果后续取得独立实验答案记录，可再做交叉核验。它不应被误写成“当前无法判定正确性”。
...
常规动态评分窗为 cue 阶段 \([0,0.8)\) 秒，以及**固定晚期窗口** \([0.8,2.8)\) 秒；后者是统一的提示锁定分析窗，不是经真实目标时刻校准的目标后窗口。候选目标周边窗为 \([t_T,t_T+0.6)\) 秒，并在 epoch 末端截断。应答前分析逐试次执行：对每个质控通过且 cue 区间内恰有一个通道9边沿的试次，使用该试次自己的端点 \(t_{\mathrm{act},i}-0.1\) 秒，分别截取累计窗 \([0,t_{\mathrm{act},i}-0.1)\) 和末 500 ms 窗；将留出“记录×cue 侧”组中各试次截取后的通道×时间样本拼接后评分。评分使用逐试次窗口的真实边界；CSV 中的窗口起止字段只汇报各试次边界的中位数。通道9只定义评分端点，不参与状态构造、模型拟合、参数选择或 EEG 预测。
...
NRMSE 越小越好；\(\Delta_H>0\) 表示在对应留出窗加入 H 后误差下降。验证按三种预处理、目标时刻候选、刺激类型、匹配分支、留出记录和提示侧保留逐条件结果。目标时刻和刺激类型是并列敏感性场景，不按测试分数挑选“真实”条件。固定晚期窗与逐试次应答前窗分别报告、分别解释；应答前评分的端点来自通道9，但通道9不进入拟合或预测。

## 8. 当前运行结果与解读

### 8.1 行为事件闭环

当前四份原始记录共 400 个 cue 试次。通道9均能按标签解码方向；通道8与通道9同号 293 次、异号 107 次，按已给规则分别判正确和错误。所有 400 次应答均在 cue+3.0 秒内出现，因此均为及时应答。400 个 cue 至下一 cue 区间均观测到通道9动作边沿；当前数据没有“区间无动作标记”的正例。cue 相对 \([-1,+5]\) 秒窗口内的 400 次声明码仍只是观测窗计数。首个连续非零动作段持续时间中位数为 1.2344 秒；该值表示动作码持续区间，不等同于从真实目标出现到动作开始的传统反应时。

当前 400 个 cue 区间均有通道9动作边沿，因此本数据没有“区间无动作标记”的正例可用于拟合该事件的分类器。cue+3.0 秒规则下的及时性可直接逐试次判定；但由于真实 target onset 缺失，传统目标相对 RT 和 DDM 仍不可靠。
...
图 3 所示通道9首次边沿总体中位数为 cue 后 2.2148 秒；按参考模型将其作为操作性应答标记时，中位 \(t_{\mathrm{act}}-100\) ms 截止点为 2.1148 秒，质控后逐试次截止点的 5%–95% 范围为 2.1148–2.1188 秒。若将候选目标 onset 2.2 秒或 2.4 秒当作真实目标时刻，它们分别比中位端点晚约 85 ms 和 285 ms。因此，在这两个候选假设下，应答前窗口完全没有覆盖目标出现后的信号，不能用该窗口验证目标引起的线索—目标匹配、冲突或控制过程。候选 2.0 秒会在中位端点前留下约 115 ms，但它仍只是未核实的时间假设；不能为了消除矛盾而把目标时刻直接改成 2.0 秒。

因此，\([0.8,2.8)\) 秒结果统一称为“固定晚期窗口分析”，应答前端点分析单列报告。当前端点使用的是通道9首次零到非零边沿；该边沿是否对应实际动作开始时刻仍需结合行为记录或同步事件资料核实。应答前评分已对每个试次分别截断于自身的 \(t_{\mathrm{act}}-100\) ms，再在留出“记录×cue 侧”组内拼接截取样本计算指标。候选目标 2.2 秒和 2.4 秒仍晚于总体中位端点约 2.115 秒；因此这些候选假设下的逐试次应答前窗口仍不含目标出现后的信号，不能用来验证目标后的匹配、冲突或控制过程。

### 8.5 应答前端点评分（逐试次）

累计应答前窗和末 500 ms 窗的逐试次留出分数保存在 `output/dynamic_heldout_validation/t_act_endpoint_cv_metrics.csv`，按预处理、候选目标情景、模型、留出记录、提示侧和窗口分别列出。每个有效试次使用自身通道9边沿减 100 ms 作为截止点，先裁切该试次 EEG，再将组内试次数值拼接后计算 RMSE、NRMSE 和相关系数。`n_test_trials_scored` 给出该留出组实际参与该分数的试次数；`window_start_s`、`window_stop_s` 及端点分位字段是试次边界摘要，并不表示评分改用公共组级端点。通道9只用于确定评分窗口。

本次逐试次应答前评分结果如下；每个单元格汇总 152 个“候选情景×留出记录×提示侧”分数行，显示均值及行间标准差。每个分数的试次数中位数为 44.5，范围为 41–49。

| 预处理 | 模型 | 累计窗 NRMSE 均值（SD） | 末 500 ms NRMSE 均值（SD） |
|---|---|---:|---:|
| 因果滤波 | Q2 visual only | 1.0264（0.0419） | 1.0591（0.0936） |
| 因果滤波 | Q2 + memory | 1.0266（0.0420） | 1.0583（0.0922） |
| 因果滤波 | Q2 + memory + control | 1.0283（0.0444） | 1.0565（0.0898） |
| 不滤波 | Q2 visual only | 1.0557（0.0534） | 1.0505（0.0526） |
| 不滤波 | Q2 + memory | 1.0537（0.0513） | 1.0494（0.0518） |
| 不滤波 | Q2 + memory + control | 1.0523（0.0501） | 1.0492（0.0517） |
| 零相位滤波 | Q2 visual only | 1.0212（0.0229） | 1.0306（0.0262） |
| 零相位滤波 | Q2 + memory | 1.0206（0.0233） | 1.0284（0.0240） |
| 零相位滤波 | Q2 + memory + control | 1.0239（0.0293） | 1.0251（0.0218） |

这些应答前 NRMSE 大多接近 1，动态模型与视觉基线之间的差异很小，未呈现记忆/控制状态稳定改善应答前预测的证据。均值汇总跨候选刺激情景和提示侧；这些重复敏感性条件不是独立样本。通道9首次零到非零边沿是否对应实际动作启动，仍需外部同步资料核实。候选目标 2.2 秒和 2.4 秒晚于典型的应答前截止点，因此这些情况下的应答前分数不能验证目标出现后的匹配、冲突或控制过程；2.0 秒场景也仍受目标时刻未核实的限制。
...
**图 3：模型分析窗口与时间窗冲突。** 标出 cue 前基线、提示阶段、固定晚期窗口、候选目标时刻，以及以通道9首次边沿 \(t_{\mathrm{act}}-100\) ms 为终点的累计/末500 ms应答前窗。图中的总体中位数和分位带用于展示时序分布；实际评分逐试次使用各自端点。按中位边沿约 2.215 秒计算，中位端点约为 2.115 秒；候选目标 2.2 秒和 2.4 秒分别晚于端点约 85 ms、285 ms，所以这些候选假设下应答前窗不含目标后的加工。2.0 秒候选虽在中位端点前约 115 ms，但真实目标时刻尚未核实。通道9首次边沿是否等于实际动作开始也待核实。
