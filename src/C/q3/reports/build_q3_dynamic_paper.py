...
ABSTRACT = """题目要求在问题二的脑电形成机制基础上构造认知宏观模型，并用实测脑电验证，同时指出应答可能错误或未及时。本文以四份256 Hz记录中的F3、Fz、F4和VisCue为观测对象，沿“前向机制衔接、动态状态构造、整份记录留出检验”建立问题三模型。事件审计发现400个提示事件各对应一个第9通道动作段，其相对提示时间中位数为2.2148 s。按已给规则，同一动作段内TgtAct的同号±1起始码后接同号±2声明码视为一次应答，首次零到非零边沿定义动作时刻；通道8与通道9同号判正确、异号判错误，400次应答均在cue后3.0 s内。未独立标记的逐试次目标时刻仍使目标相对反应时未知。通道9只用于行为事件标注及应答前窗口定位，不进入EEG特征或状态预测。质量筛选后保留355个完整试次，其中按同一规则256次正确、99次错误，均在cue后3.0 s内应答。问题二前向链在候选提示与目标场景下生成五个源代理，并经3×5导联矩阵正向投影到三电极；数值投影的最大绝对差为5.954×10⁻⁸，该检查只确认计算接口一致。本文以问题二早期视觉兴奋群体均值驱动视觉状态V，再以提示门控、候选目标门控和假设匹配证据递推记忆相关状态H与控制状态P。目标类型、目标时刻、持续时间及匹配证据均按候选情景设置，状态时间常数和增益保持预设值，不由留出数据反向挑选。观测方程按电极逐一拟合问题二视觉轨迹、H、P及控制交互项；输入标准化量和岭回归系数只由训练记录估计。每折完整留出一份记录，用其余三份记录的记录×提示侧均值波形拟合，再在固定提示后0.8–2.8 s晚期窗评价，比较训练均值模板、问题二视觉基线、加入记忆态及再加入控制态四种方案；同时考察无滤波、因果滤波、零相位滤波和2.0、2.2、2.4 s三个目标候选时刻。加入H后，晚期NRMSE在因果滤波下由1.6479降至1.6474，变化仅0.03%；无滤波和零相位滤波下分别增加0.78%和1.31%。在九个处理方式×目标时刻聚合单元中只有两个误差下降，控制态也没有形成跨处理方式的额外收益。所有处理口径下误差仍高于留出信号标准差，且外层只有四份记录。因此，当前证据支持“模型已构造并完成跨记录检验”，但不支持记忆态具有稳定预测增益的结论。结果不作脑区定位、临床诊断效能或真实行为决策证据；候选目标时刻不确定导致目标后阶段解释和目标相对反应时仍受限。"""
...
def render_and_report(markdown: str, tex: str, summary: dict) -> dict:
    ...
    generation_report = {
        ...
        "pdf_pages": page_line,
        "limitations": ["four record folds only", "participant independence unverified", "per-trial target onset unobserved so target-relative RT unavailable", "behavior labels are not inputs to the EEG dynamic model", "state parameters fixed"],
    }
